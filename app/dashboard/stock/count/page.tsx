...
export default function StockCountPage() {
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        ...
        const countItems: StockCountItem[] = products.map((p) => ({
          id: p.id,
          name: p.name,
          sku: p.code,
          category: p.categoryName || p.category || "General",
          systemQty: p.current_stock ?? p.stock ?? 0,
          physicalQty: p.current_stock ?? p.stock ?? 0,
          unit: "pcs",
        }))
        setItems(countItems)
      } catch (err) {
        console.error("Failed to load products for stock count:", err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleEdit = (id: number, currentVal: number) => {
    setEditingId(id)
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Log physical stock counts and compare them with the system quantities from MySQL</p>
          </div>
        </div>
      </div>
      ...
      <Card className="border bg-card shadow-2xs">
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <CardDescription>Enter actual quantities counted by staff in the warehouse</CardDescription>
          </div>
